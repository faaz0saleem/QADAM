# QADAM — Project Brief for Claude Code
...
## 14. IMPLEMENTATION NOTES (maintained by Claude Code)

Decisions made while building that a future session needs to know. Additions to this
section are notes, never amendments — §0–§13 above are the brief and are not edited.

- **Money is integer PKR everywhere.** No floats, no decimals, no minor units. PKR has no
  circulating subunit. Column names carry a `_pkr` suffix so a unitless integer never
  reaches a money context by accident. `app_config.COIN_VALUE_PKR` is the one numeric.
- **Every ledger row carries the expiry of the batch it belongs to.** Credits open a batch;
  debits are allocated FIFO (earliest-expiring first) and split into one row per batch they
  draw from. This makes `SUM(delta) WHERE expires_at > now()` (§5) correct as written, and
  makes §7.2's "2,400 coins expiring in 11 days" a straight `GROUP BY expires_at` — spent
  coins stop counting toward an expiry batch the moment they are spent.
- **§0 is enforced at two levels.** Per item, by the `discount_within_margin` CHECK from
  §3.2. Per order, by a deferred constraint trigger that re-derives the order header from
  its items at COMMIT and refuses any order whose gross profit is negative. The second
  level exists because `orders.total_pkr` is what actually gets charged — a per-item check
  alone leaves the order header as an unguarded path to the same loss.
- **Coin-backed discounts.** `orders.coin_value_pkr` snapshots the conversion rate at
  placement so `discount_pkr = floor(coins_spent * coin_value_pkr)` can be a CHECK
  constraint that never retroactively breaks when the rate is tuned (§4).
- **Soft delete only for users with ledger history.** `coin_ledger` is append-only by
  trigger, so a cascading hard delete would be refused. Use `users.status = 'deleted'`.
- **The local-parity migration** (`..._bootstrap_local_parity.sql`) creates the Supabase
